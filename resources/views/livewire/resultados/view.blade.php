		...
		<div class="col-md-12">
			<div class="card">
				<div class="card-header">
					<div style="display: flex; justify-content: space-between; align-items: center;" class="row">
						...
						<div class="col-md-2 mb-2">
							<div wire:poll.4s class="btn btn-sm btn-success btn-block" style="margin-top:0px; margin-bottom:0px;">
								{{ session('message') }}
							</div>
						</div>
						@endif
						...
						<div class="col-md-3 mb-2">
							<input wire:model='keyWord' type="text" class="form-control" name="search" id="search" placeholder="Buscar Resultados">
						</div>
						<div class="col-md-2 mb-2">
							@if($crear == 1)
							<div class="btn btn-sm btn-info float-right" data-toggle="modal" data-target="#createDataModal">
								<i class="fa fa-plus"></i>  Añadir Resultados
							</div>
							@endif
						</div>
					</div>
				...
				<div class="card-body p-0">
						@include('livewire.resultados.create')
						@include('livewire.resultados.update')
						@include('livewire.resultados.agregar')
						@include('livewire.resultados.show')
					...
					<table class="table table-striped table-sm">
						...
						<tbody>
							...
							<tr>
								...
								<td width="150px" class="text-right">$ {{ number_format($row->presupuesto, 2, ',', '.') }}</td>
								<td>
									@if($row->estado == 1)
								...
								<td width="200px">
									...
									@endif
									@if($borrar == 1)
									<button class="btn btn-default btn-sm" onclick="confirm('Confirma borrar el registro Resultado id {{$row->id}}? \nDeleted Resultados cannot be recovered!')||event.stopImmediatePropagation()" wire:click="destroy({{$row->id}})"><i class="fa fa-trash"></i></button>
									@endif
								</td>
							@endforeach
						</tbody>
